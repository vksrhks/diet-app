...
export default function DashboardClient({ initialData }: { initialData: { dailyRecords: any[], inbodyRecords: any[] } }) {
  ...
  // 1인 선택 모드 (디폴트: 이수정 'B')
  const [selectedTodayUser, setSelectedTodayUser] = useState<'A' | 'B'>('B');
  ...
  const userB_db = initialData.dailyRecords.find((r: any) => r.userId === 'user-b')?.user || initialData.inbodyRecords.find((r: any) => r.userId === 'user-b')?.user;
  const [nameA, setNameA] = useState(userA_db ? userA_db.name : "정승후");
  const [nameB, setNameB] = useState(userB_db ? userB_db.name : "이수정");
